module LTerm_scroll_impl = struct

  open CamomileLibrary
  open LTerm_geom

  class t = LTerm_widget.t
...
module Button = struct
  ...
  class button ?(brackets=("< "," >")) initial_label =
    ...
    object(self)
    inherit LTerm_widget.t "button" as super
      ...
      self#on_event
        (function
           | LTerm_event.Key { control = false; meta = false; shift = false; code = Enter } ->
               LTerm_widget_callbacks.exec_callbacks click_callbacks ();
               true
           | LTerm_event.Mouse m when m.button = Button1 ->
               LTerm_widget_callbacks.exec_callbacks click_callbacks ();
               true
    ...
    method private apply_style ctx focused =
      let style =
        if focused = (self :> LTerm_widget.t)
        then focused_style
...
module Frame = struct
  ...
  class frame = object(self)
    inherit LTerm_widget.t "frame" as super
    ...
    method set : 'a. (#LTerm_widget.t as 'a) -> unit = fun widget ->
    ...
    method draw ctx focused =
      ...
      if size.rows >= 1 && size.cols >= 1 then begin
        ...
        else draw_frame_labelled ctx rect ~alignment:align label connection);
